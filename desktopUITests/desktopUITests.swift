...
final class desktopUITests: XCTestCase {
    ...
    func testWeChat() throws {
        ...
        // 2.登录微信
        if targetApp.buttons["Enter Weixin"].waitForExistence(timeout: 10) {
            targetApp.buttons["Enter Weixin"].click()
        }else if targetApp.buttons["Log In"].waitForExistence(timeout: 10) {
     ...
     */
    func testWhatsApp() throws {
        
        let targetApp = XCUIApplication(bundleIdentifier: "net.whatsapp.WhatsApp")
        targetApp.launch()
        ...
        sleep(3)
        var predicate = NSPredicate(format: "label CONTAINS[c] 'Search'")
        
        let searchButton = targetApp.textViews.element(matching: predicate).firstMatch
        searchButton.waitForExistence(timeout: 10)
        let firstChatPos = searchButton.coordinate(withNormalizedOffset:.zero).withOffset(CGVector(dx: 100, dy: 100))
        firstChatPos.click()
        ...
        // 6.获取编辑框
        predicate = NSPredicate(format: "label CONTAINS[c] 'Compose message'")
        let textEditor : XCUIElement = targetApp.textViews.element(matching: predicate)
        let textEditorW : CGFloat = textEditor.frame.width
        
        
        /** TEST1:          选中中间的"你好呀",然后弹窗进行翻译，翻译后替换原来中间的”你好呀“，并检测正确性   -----------------    **/
        // TEST1.1.全选已有内容
        let pos_begin = textEditor.coordinate(withNormalizedOffset: .zero).withOffset(CGVector(dx:3,dy:6  ))
        let pos_end = textEditor.coordinate(withNormalizedOffset: CGVector(dx: 0.8, dy: 0.4 ))
        /** TEST1:          选中中间的"你好呀",然后弹窗进行翻译，翻译后替换原来中间的”你好呀“，并检测正确性   -----------------    **/
        // TEST1.1.全选已有内容
        
        pos_end.doubleClick()
        pos_end.click(forDuration: 1, thenDragTo: pos_begin)
        ...
        let pos4W = Int(textEditorW * 0.8)
        let pos4 = textEditor.coordinate(withNormalizedOffset:.zero).withOffset(CGVector(dx: pos4W, dy: Y))
        // TEST1.2.选中部分文字进行弹窗翻译后插入，选中的是"好呀  你"，翻译后应该满足 "你好呀  你.+好呀  "
        pos1.click(forDuration: 1, thenDragTo: pos2)
        ...
        // TEST3.1.获取最后一条聊天记录
        predicate = NSPredicate(format: "label CONTAINS[c] 'Your message,'")
        let lastMessageLabel = targetApp.otherElements.matching(predicate).allElementsBoundByAccessibilityElement.last
        let lastMessageLabelFrame = lastMessageLabel?.frame
        let endX = lastMessageLabelFrame!.width - 83
        let midX = lastMessageLabelFrame!.width - 150
        let startX = endX - 220
        ...
        // TEST3.2.选中
        lastMessageLabel?.click()
        
        lastMessageLabel!.coordinate(withNormalizedOffset: .zero).withOffset(CGVector(dx:endX,dy: midY)).click(forDuration: 0.2, thenDragTo: lastMessageLabel!.coordinate(withNormalizedOffset: .zero).withOffset(CGVector(dx:startX,dy: midY)))
